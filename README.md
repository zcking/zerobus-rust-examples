...
A collection of polyglot example applications demonstrating how to use the [Databricks Zerobus SDK](https://github.com/databricks/zerobus-sdk-rs) across different programming languages.
...
zerobus-generate \
  ...
  --table $TABLE_NAME \
  --output-dir hello-world/proto
```

**Note:** With the buf-based workflow, `zerobus-generate` only creates the `.proto` file. The Rust bindings and descriptor files are generated by buf during the build process.
